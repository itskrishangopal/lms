...
<html lang="en">
  ...
  <div class="container-fluid">
    ...
    <h1 style="text-align: center; padding-top: 40px; font-family: sans-serif;"><strong>Issue Book</strong></h1>
    <br>  
    
  <div style="padding-left: 20px; padding-right: 20px;">
    <table class="table table-bordered border-dark">
      <tr class="table-primary">
        <th scope="col"><center>Issuing to</center></th>
...
<?php
  ...
  $sql = "SELECT issueto,bid,bname,bauthor,yop,Time FROM bookissue";
  //$sql = "SELECT issueto,bid,bname,bauthor,yop,Time,count(bname) as quantity FROM `bookissue` group by bname HAVING COUNT(bname)>=1";
  $result = $conn->query($sql);
   
   if($result->num_rows > 0) {
     while($row = $result->fetch_assoc()) {
       echo "<tr><td><center><strong>". $row["issueto"] ."</strong></center></td><td><center>". $row["bname"] ."</center></td><td><center>". $row["bauthor"] ."</center></td><td><center>". $row["yop"] ."</center></td><td><center>". $row["Time"] ."</center></td><td><center><a href='miscadminissuebook.php?rn=$row[bid]''><button>Issue</button></a></center></td></tr>";
      }
      echo "</table>";
    }
    else {
